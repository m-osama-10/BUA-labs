...
import { devices, faculties, departments, laboratories } from "./drizzle/schema";
...
async function seed() {
  try {
    ...
    if (!faculty) {
      ...
      faculty = { id: (insertResult as any).insertId || 1, name: "Science", code: "SCI" };
    }
    ...
    if (!dept) {
      ...
      dept = { id: (insertResult as any).insertId || 1, facultyId: faculty.id, name: "Pharmacology", code: "PHARM" };
    }
    ...
    for (const labName of labNames) {
      ...
      if (!lab) {
        ...
        lab = { id: (insertResult as any).insertId || 1, departmentId: dept.id, name: labName, code: code };
      }
      labs[labName] = lab;
