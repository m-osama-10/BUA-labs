...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    // Check Authorization header first (for dev login)
    ...
    if (authHeader && authHeader.startsWith("Bearer ")) {
      ...
      // Regular authentication flow - check cookies
      const cookies = this.parseCookies(req.headers.cookie);
      sessionCookie = cookies.get(COOKIE_NAME);
      console.log("[Auth] Token from cookie:", sessionCookie ? "Found" : "Not found");
    }
    ...
    // Standard production flow - get user from database
    ...
    try {
      ...
      console.warn("[Auth] DB lookup failed:", dbErr);
      // In development, fall back to a temporary in-memory admin user so
      // local-login works even when the database is down.
      if (!ENV.isProduction) {
        console.warn("[Auth] Falling back to development user due to DB failure");
        user = {
          id: 999,
    ...
    try {
      ...
      console.warn("[Auth] Failed to update last signed in:", dbError);
      // Continue anyway
    }
